...
public class DataPurgerServiceTest {
  ...
  private void createTestPurgePartitionFiles(FileSystem fs, Cluster cluster,
      Calendar date, HCatClient hcatClient)
          throws Exception {
    
    
    for(String streamname: cluster.getSourceStreams()) {
     // HCatClient hcatClient = hcatUtil.getHCatClient();
     /* String tbaleName = "";
      HCatClient hcatClient = null;
      TestHCatUtil.addPartition(hcatClient, "", "", "", partSpec);
    ...
    */  
      ...
      HCatAddPartitionDesc hcatPartDesc = HCatAddPartitionDesc.create("conduit",
          "conduit_local_"+streamname, commitpath, partSpec).build();
     hcatClient.addPartition(hcatPartDesc);
     LOG.info("AAAAAAAAAAAAAAAAAAAAAAAAAAAAA hcatpartdec   :::: " + hcatPartDesc);
     LOG.info("AAAAAAAAAAAAAAAAAAAAA list of parts in datapurger :  " + hcatClient.getPartitions("conduit", "conduit_local_"+streamname));
  ...
  private void verifyPurgePartitionFiles(FileSystem fs, Cluster cluster, Calendar date,
      boolean checkexists, boolean checktrashexists, HCatClient hcatClient) throws Exception {
    for (String streamname : cluster.getSourceStreams()) {
      String datapath = Cluster
          .getDateAsYYYYMMDDHHMNPath(date.getTime());
      String commitpath = cluster.getLocalFinalDestDirRoot() + File.separator
      ...
      LOG.info("AAAAAAAAAAAAAAAAAAAAA list of partitions after dropping in datapurger :  " + hcatClient.getPartitions("conduit", "conduit_local_"+streamname));
  ...
  public void testDataPurgerParittion() throws Exception {
    ...
    for (Cluster cluster : config.getClusters().values()) {
     ...
     for (String stream : cluster.getSourceStreams()) {
       TestHCatUtil.createTable(hcatClient, "conduit", "conduit_local_"+stream, TestHCatUtil.getPartCols());
       /*HCatCreateTableDesc hcatTbDesc = HCatCreateTableDesc.
           create("conduit", "conduit_local_"+ stream, TestHCatUtil.getPartCols()).build();
       hcatClient.createTable(hcatTbDesc);
       */LOG.info("AAAAAAAAAAAA table created : " + hcatClient.getTable("conduit", "conduit_local_"+stream));
     }
      ...
      createTestPurgefiles(fs, cluster, date3, true);
*/
      TestDataPurgerService service = new TestDataPurgerService(config, cluster, hcatUtil1);
