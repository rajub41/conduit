...
public class DynamicInputFormat<K, V> extends InputFormat<K, V> {
  ...
  private List<DynamicInputChunk> splitCopyListingIntoChunksWithShuffle
    ...
    SequenceFile.Reader reader
            = new SequenceFile.Reader(listingPath.getFileSystem(configuration),
                                      listingPath,
                                      configuration);
      ...
      while (reader.next(relPath, fileStatus)) {
        if (recordCounter % (nChunksOpenAtOnce*numEntriesPerChunk) == 0) {
          ...
          openChunks = createChunks(
                  chunkSet, chunkCount, nChunksTotal, nChunksOpenAtOnce);
  ...
  private static void closeAll(List<DynamicInputChunk> chunks) throws IOException {
    for (DynamicInputChunk chunk: chunks)
      chunk.close();
  }

  private static List<DynamicInputChunk> createChunks(DynamicInputChunkSet chunkSet,
    ...
    for (int i=chunkCount; i < chunkIdUpperBound; ++i)
      chunks.add(createChunk(i, chunkSet));
    return chunks;
  }

  private static DynamicInputChunk createChunk(int chunkId,
                           DynamicInputChunkSet chunkSet) throws IOException {
    return chunkSet.createChunkForWrite(String.format("%05d", chunkId));
  }
  ...
  private int getNumEntriesPerChunk(Configuration configuration) {
    return DistCpUtils.getInt(configuration,
                              CONF_LABEL_NUM_ENTRIES_PER_CHUNK);
  }
